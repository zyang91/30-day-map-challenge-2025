# Day 12 of 30 Day Map Challenge - 2125

**Day 12 (2125)**: I created a speculative visualization of Philadelphia in the year 2125, imagining how the city's relationship with water bodies and green spaces might evolve. This map projects a future where Philadelphia has expanded its water buffers by 100 meters and park access zones by 500 meters, representing a hypothetical scenario of enhanced environmental planning and climate adaptation.

The visualization overlays two key spatial assumptions on Philadelphia's boundary: expanded water buffers (shown in blue) around all water bodies, and enlarged park access zones (shown in green) around existing park facilities. These buffers symbolize potential future policies for flood management, ecosystem preservation, and equitable green space access.

![](day12-2125.png)
Explore the interactive map through the link [here](https://zyang91.github.io/30-day-map-challenge-2025/day12-2125.html).
**Key Assumptions:**
- **Water Buffers**: 100-meter buffer zones around all water bodies in Philadelphia, representing potential future floodplain management, riparian restoration, or climate adaptation strategies
- **Park Access Zones**: 500-meter buffer zones around all parks and park facilities, representing enhanced green infrastructure and equitable access to recreational spaces

**Technical Implementation:**
- **sf (Simple Features)** - R package for handling spatial vector data and geometric operations
- **tidyverse** - R's ecosystem of packages for data manipulation and visualization
- **leaflet** - R package for creating interactive web maps
- **htmltools** - R package for HTML generation in R
- **ggplot2** - R's powerful data visualization package (via tidyverse)

**Data Sources:**
- **Philadelphia Boundary**: [Philadelphia City Limits](https://opendata.arcgis.com/datasets/405ec3da942d4e20869d4e1449a2be48_0.geojson) - City boundary data from Philadelphia Open Data Portal
- **Water Bodies**: [Water Features Dataset](https://hub.arcgis.com/api/v3/datasets/2b10034796f34c81a0eb44c676d86729_1/downloads/data?format=geojson&spatialRefId=4326) - Water body geometries from Philadelphia Open Data Portal
- **Parks & Facilities**: [City Facilities Dataset](https://hub.arcgis.com/api/v3/datasets/b3c133c3b15d4c96bcd4d5cc09f19f4e_0/downloads/data?format=geojson&spatialRefId=4326) - Park locations from Philadelphia Open Data Portal

**Coordinate Reference System:**
- `EPSG:2272` (Pennsylvania State Plane South, NAD83, US Feet) for accurate spatial analysis and buffering operations
- `EPSG:4326` (WGS84) for web map display in Leaflet
